...
export default async function registerUserHandler(req, res) {
    ...
    try {
        ...
        // Send registration notifications via Brevo
        try {
            const { apiKey, senderEmail, adminEmail } = await getAdminEmailConfig(signature);
            const brevo = new BrevoClient({ apiKey });

            const userHtml = `<div style="font-family:Arial,sans-serif;padding:20px;">
                <h2>Welcome to ${platformName}</h2>
                <p>Hello ${newRow.firstname},</p>
                <p>Your account is ready. <strong>Account Number:</strong> ${acctNo}</p>
            </div>`;

            const adminHtml = `<div style="font-family:Arial,sans-serif;padding:20px;">
                <h3>New User Registration</h3>
                <p><strong>Name:</strong> ${newRow.firstname} ${newRow.lastname}</p>
                <p><strong>Email:</strong> ${newRow.email}</p>
                <p><strong>Account Number:</strong> ${acctNo}</p>
            </div>`;
