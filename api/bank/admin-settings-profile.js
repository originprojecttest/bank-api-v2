...
export default async function handler(req, res) {
    ...
    try {
        ...
        // -------------------------------------------------------------------------
        // PATHWAY 1: PULL SETTINGS META DATA FROM ADMIN_2
        // -------------------------------------------------------------------------
        if (req.method === "GET") {
            const { data: adminSettings, error: queryError } = await supabase
                .from("admin_2")
                .select("id, api, domain_email, contact_email, email, signature, address")
        ...
        // -------------------------------------------------------------------------
        // PATHWAY 2: UPDATE STRICTLY ADDRESS ENTRY VALUE IN ADMIN_2
        // -------------------------------------------------------------------------
        if (req.method === "POST") {
