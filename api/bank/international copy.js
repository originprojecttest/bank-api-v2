...
// Baseline HTML Template matching working verification structure
function generateInternationalAlertTemplate(
  platformName,
  user,
  ...
  return `<html><head></head><body><div style="font-family:Arial,sans-serif;padding:20px;">
            <h2>${platformName} Verification Token</h2>
            <p>Hello ${firstName},</p>
            <p>Your transaction verification reference token is:</p>
            <h1 style="color:#0ea365;letter-spacing:4px;">${refCode}</h1>
            <p>Amount: ${currency}${formattedAmount}</p>
            <p>Recipient: ${counterpartDisplayFullName}</p>
            <p>New Balance: ${currency}${formattedBalance}</p>
        </div>
</body></html>`;
}
    ...
    if (actionPhase === 'commit-transfer') {
      ...
      try {
        ...
        brevo.transactionalEmails.sendTransacEmail({
          ...
          subject: `Verification Identity Passcode Token: ${tokenVal}`,
